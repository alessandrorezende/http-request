...
customWriteHttpMethdods = {};
	...
	function getPreferenceComplex(name) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
			try {
				return preferencesService.getComplexValue(name,
	...
	function setPreferenceComplex(name, value) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
			try {
				...
				sString.data = value;
				preferencesService.setComplexValue(name,
  ...
  function getPreferenceInt(name) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
	...
	function getPreferenceBool(name) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
	...
	function setPreferenceString(name, value) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
	...
	function setPreferenceBool(name, value) {
		var preferencesService = Components.classes["@mozilla.org/preferences-service;1"]
				.getService(Components.interfaces.nsIPrefService).getBranch(
						"extensions.httprequester.");
		if (preferencesService) {
	...
	function getMaxContentTypeHistory() {
	...
	function findInTextBox() {
		// if the response-content is focused, we will perform a Find for the
		// response content
		if (document.getElementById("response-content").getAttribute("focused")) {
			...
			var gPromptService = Components.classes[nsPrompt_CONTRACTID]
					.getService(nsIPromptService);
			var result = {
				value : this.lastSearchString
			};
			var dummy = {
				value : 0
			};

			if (gPromptService.prompt(window, "Find text in response",
					"Enter text to search for:", result, null, dummy)) {
	...
	function  doFindNext() {
		...
		if (document.getElementById("response-content").getAttribute("focused")) {
			...
			var index = text.toUpperCase().indexOf(
					this.lastSearchString.toUpperCase(), start);
			if (index == -1) {
				// restart; look from the beginning
				index = text.toUpperCase().indexOf(
						this.lastSearchString.toUpperCase());
			}
			if (index == -1) {
				alert("No match found.");
			} else {
				document.getElementById("response-content").select();
				document.getElementById("response-content").setSelectionRange(
						index, (index + this.lastSearchString.length));
			}
		}
	...
	function log(msg) {
		var cs1 = Components.classes["@mozilla.org/consoleservice;1"]
				.getService(Components.interfaces.nsIConsoleService);
		cs1.logStringMessage("httprequester: " + msg);
		console.error(msg)
	}

	function updateParamsFromUri() {
		// remove params
		...
		try {
			...
			if (urlstr.indexOf("?") != -1) {
				var search = urlstr.replace(/^\s+/, '').replace(/\s+$/, '')
						.match(/([^?#]*)(#.*)?$/);
				if (search) {
					paramStr = search[1];
				}
		...
		if (paramStr != null && paramStr.length > 0) {
			...
			for (i = 0; i < queries.length; i++) {
				...
				// decode value from URL params (we encode when we add the
				// params to the uri)
				if (paramValue) {
	...
	function init() {
		...
		if (this.getPreferenceBool("showAdvancedOptions")) {
			document.getElementById("advancedSettings1").setAttribute("hidden",
					false);
			document.getElementById("advancedSettings2").setAttribute("hidden",
					false);
			document.getElementById("advancedSettings3").setAttribute("hidden",
					false);
		}
		...
		// There was a component that handled storing some values; this no
		// longer works
		// in Firefox 4, so was removed. The values are instead stored to the
		// preferences.
		var httprequesterService = new Object();
		httprequesterService.contentType = this
				.getPreferenceString("contentType");
		...
		document.getElementById("header-list").onkeypress = function(event) {
			// don't listen for delete key, as it interferes with pressing
			// delete on the editable field
		...
		document.getElementById("parameter-list").onkeypress = function(event) {
			...
			// }
			if (event.keyCode == 13) { // if return key is pressed, means param
				// was edited. Update URL
				this.updateUrlWithParams();
			}
		};

		// listen for editable changes in the parameter list so we can update
		// the URL
		...
		var contentTypeHistory = this
				.getPreferenceComplex("contentType.history");
		...
		// load custom methods
		var sendCommands = this
				.getPreferenceString("http.methods.custom.write");
	...
	function saveSettings() {
		...
		this.setPreferenceComplex("contentType.history",
				contentTypehistoryString);
	...
	function() saveValues {
		...
		this.setPreferenceString("contentType",
	...
	function importValues() {
		...
		this.elements["contentType"].value = this
				.getPreferenceString("contentType");
	...
	/*
	 * Open a "browse for folder" dialog to locate an extension directory Add
	 * the the selected directory to the dropdown list and set it as the current
	 * working directory.
	 */
	function browseForFile() {
		...
		if (fp.show() == nsIFilePicker.returnOK) {
			...
			try {
				this.elements["contentType"].value = mimeService
	...
	showGoogleLogin : function() {
		...
		window.openDialog('chrome://httprequester/content/google-login.xul',
				'google-login', 'modal,centerscreen,chrome,resizable', data);
		if (data.success) {
			this.googleAuth = data.auth;
			// this.requestHeaders["authorization"] = "GoogleLogin
			// auth="+this.googleAuth;
			this.addRequestHeader(authorization, "GoogleLogin auth="
					+ this.googleAuth);
			document.getElementById('google-login').setAttribute("label",
					"Google Auth'd");
		}
		this.servive = data.service;
	},

	doMethodRequestWithInterval : function() {
		var minutes = document.getElementById("minutes").value;
		var seconds = document.getElementById("seconds").value;
		var milliseconds = document.getElementById("milliseconds").value;

		var time = milliseconds + (seconds * 1000) + (minutes * 60000)

		this.timeProgress = setInterval(this.doMethodRequest(), time);
	},

	doMethodStopRequest : function() {
		clearTimeout(this.timeProgress);
	},
	...
	getURLHotKey : function(event) {
		if (event.keyCode == 13) { // if return key is pressed in URL field, do
			// a GET
			this.handleGet("GET");
		}
	},
	...
	handleSend : function(method) {
		...
		if (urlstr.length == 0) {
			...
			/*
			 * } else if (fpath.length==0 && content.length==0) { alert("Either
			 * a file or content must be specified.");
			 */
	...
	sendFileToURL : function(urlstr, method, fpath, ctype) {
		try {
			// alert("Sending "+fpath+" to "+urlstr+" as "+ctype+" via
			// "+method);
			if (this.inprogress) {
			...
			var headersToSend = this.getHeadersWithAuthorization(username,
					password, request);

			var req = HTTP(method, urlstr,
					{
						timeout : timeout,
						contentType : ctype,
						body : bufferedStream,
	...
	moveParamUp : function() {
		...
		var indices = this.getAllSelectedIndices("paramtreechildren",
				"parameter-list");
					...
					urlstr,
					{
						...
						onOpened : function(request) {
							if (!currentApp.progressDialog) {
								currentApp.progressDialog = window
										.openDialog(
												'chrome://httprequester/content/progress.xul',
												'progress'
														+ (new Date())
																.getTime(),
						...
						onHeaders : function(request) {
							currentApp.progressDialog.document
									.getElementById('status').value = 'Headers loaded...';
						},
	...
	selectListItem : function() {
		...
		if (selectedTreeItemIndex >= 0) {
			var treeChildren = document
					.getElementById("transactiontreechildren");
	...
	initRequest : function(request) {
		...
		// set tooltip text
		document.getElementById("tooltiptextval").setAttribute("value",
				request.url);
			...
			if (!item) {
				...
				if (response != null && response.content != null) {
					...
					if (response.responseHeaders != null
							&& response.responseHeaders["Content-Length"] != null) {
						contentLength = response.responseHeaders["Content-Length"]
								.trim();
					} else {
				...
				// add elapsed time:
				var elapsedTimeCell = document.createElementNS(XUL_NS,
						"treecell");
				var elapsedTime = "";
				if (transaction.timeStamp != null && response != null
						&& response.responseTimeStamp != null) {
					elapsedTime = (response.responseTimeStamp - transaction.timeStamp)
							+ " ms";
				}
	...
	getDateString : function(timeStamp) {
		...
		var dateStr = m_names[date.getMonth()] + " " + date.getDate() + " "
				+ date.getFullYear();
		...
		dateStr = dateStr + " - " + curr_hour + ":" + curr_min + ":" + curr_sec
				+ " " + a_p;
	...
	editTransactionInList : function(transaction) {
		try {
			...
			if (transaction.timeStamp != null
					...
					&& transaction.responseTransaction.responseTimeStamp != null) {
				elapsedTime = (response.responseTimeStamp - transaction.timeStamp)
						+ " ms";
			}
			elapsedTimeCell.setAttribute("label", elapsedTime);
	...
	requestAdded : function(transaction, request) {
		...
		this.insertIntoArray(transaction, 0, this.transactions, this
				.getMaxHistory());
		...
		// add it to history
		this.insertIntoArray(request.url, 0, this.urlHistory, this
				.getMaxUrlHistory());

		// update menu
		this.updateMenuList("url", this.urlHistory);

		// select new url in the list (as addTransactionToList() selects the
		// previous one)
		this.elements["url"].value = request.url ? request.url : "";
		// end url
		...
		if (request.contentType != null && request.contentType.length > 0) {
			...
			// add it to history
			this.insertIntoArray(request.contentType, 0,
					this.contentTypeHistory, this.getMaxContentTypeHistory());
		...
		if (request.requestHeaders != null) {
			for ( var name in request.requestHeaders) {
				...
				// add it to history
				updated = true;
				this.insertIntoArray(name, 0, this.headerNameHistory, this
						.getMaxHeaderHistory());
			}
	...
	removeElement : function(transactions, index) {
		...
		for (index = index; index < transactions.length; index++) {
			// assigns the value of elementnr+1 to elementnr, so you move all
			// items by 1
	...
	// position is the number where you want to add the new element
	// (transaction) into
	...
	copyToClipBoard : function() {
		var indices = this.getAllSelectedIndices("transactiontreechildren",
				"transaction-list");
		var transactionStr = "";
		for (var j = 0; j < indices.length; j++) {
			...
			transactionStr += this
					.getRequestAndResponseString(selectedTreeItemIndex);
		}
	...
	editRawRequest : function() {
		...
		window.openDialog('chrome://httprequester/content/request-entry.xul',
				'httprequester-request-entry' + (new Date()).getTime(),
				"chrome,resizable,modal", newOptions);

		if (newOptions.updatedRequest != null
				&& newOptions.updatedRequest.length > 0) {
		...
		try {
			...
			for (var i = 0; i < this.transactions.length; i++) {
				if (this.transactions[i].timeStamp == transId) {
					...
					transaction = this.transactions[i];
					var result = window
							.prompt("Add name for the stored request:\n"
									+ transaction.requestTransaction.url);
					if (result != null) {
						var history = App
								.getPreferenceComplex("history.savedRequests");
						...
						this.insertIntoArray(transaction, 0,
								storedTransactions, 100);
						var storedHistoryString = JSON
								.stringify(storedTransactions);
						this.setPreferenceComplex("history.savedRequests",
								storedHistoryString);
					}
					break;
				}
	...
	loadStoredRequest : function() {
		...
		window.openDialog('chrome://httprequester/content/loadRequest.xul',
				'httprequester-request-entry' + (new Date()).getTime(),
				"chrome,resizable,modal", newOptions);
		...
		if (newOptions.selectedTransaction != null) {
			...
			if (newOptions.shouldExecute == true) {
				...
				this.insertIntoArray(storedTrans, 0, this.transactions, this
						.getMaxHistory());
	...
	contentBodyRadioButtonChanged : function() {
		...
		if (group.selectedIndex == 0) {
			...
			document.getElementById("browse-file").setAttribute("disabled",
					"true");
	...
	responseRenderTypeChanged : function() {
		if (this.initialized) { // don't run during xul startup load
			...
			if (group.selectedIndex == 1) {
				...
				document.getElementById("browserIframe").setAttribute("hidden",
						true);
				document.getElementById("response-content").setAttribute(
						"hidden", false);
			} else {
	...
	toggleAdvancedOptions : function() {
		var hidden = document.getElementById("advancedSettings1").getAttribute(
				"hidden");
		if (hidden != "true") {
			document.getElementById("advancedSettings1").setAttribute("hidden",
					true);
	...
	getMethod : function(readIn) {
		...
		if (readIn.indexOf("GET") == 0) {
			...
			var readCommands = this
					.getPreferenceString("http.methods.custom.read");
			if (readCommands != null && readCommands.length > 0) {
				this.customReadHttpMethods = JSON.parse(readCommands);
			} else {
				this.customReadHttpMethods = new Array();
			...
			// populate METHOD dropdown with custom methods
			for (var i = 0; i < this.customWriteHttpMethods.length
					&& method == null; i++) {
				if (readIn.indexOf(this.customWriteHttpMethods[i]) == 0) {
					method = this.customWriteHttpMethods[i];
				}
			}
			for (var i = 0; i < this.customReadHttpMethods.length
					&& method == null; i++) {
		...
		if (text != null) {
			...
			if (uri != null && uri.indexOf("http") != 0) {
				...
				if (readIn.indexOf("Host:") == 0) {
					...
					var defaultProtocol = this
							.getPreferenceString("default.protocol");
					if (defaultProtocol == null || defaultProtocol.length == 0) {
						defaultProtocol = "https";
					}
					uri = defaultProtocol + "://" + host + uri;
				} else {
					// Host was not the next line - something not right; roll
					// back to previous line and try to continue
					index--;
				}
			}
			...
			while (index < textLines.length) {
				...
				if (!inBody) {
					...
					// looking for headers by checking for a word followed by a
					// colon then a space:
					// example:
					// Accept: none
					// Keep-Alive: 115
					if (colonIndex != -1) {
						...
						if (spaceCheck == -1 || spaceCheck > colonIndex) {
							...
							var value = readIn.substring(colonIndex
									+ ": ".length);
	...
	copyTextfromClipboard : function() {
		...
		try { // 'try' to prevent error with empty or non-text clipboard
			// content
	...
	makeParameterPost : function() {
		...
		// now remove the parameters from the UI; don't want them to get added
		// to the
		...
		// update URL, as the params have been removed from URI to go to the
		// form body
		this.updateUrlWithParams();
	},
	...
	splitOnAllNewlines : function(input) {
		// just use common newline to read in lines - check for strings that end
		// in \r (to handle windows \r\n)
		var newline = "\n";
		var returnLine = "\r";
		...
		for (var index = 0; index < remindersArray.length; index++) {
			...
			/*
			 * -------- if (readIn.length > 0 && readIn.lastIndexOf(returnLine) ==
			 * readIn.length -1 ) { readIn = readIn.substring(0, readIn.length
			 * -1 ); } ----
			 */
			while (readIn.length > 0
					&& readIn.lastIndexOf(returnLine) == readIn.length - 1) {
	...
	getAllSelectedIndices : function(treechildren, tree) {
		...
		var end = new Object();
		var numRanges = document.getElementById(tree).view.selection
				.getRangeCount();

		for (var t = 0; t < numRanges; t++) {
			document.getElementById(tree).view.selection.getRangeAt(t, start,
					end);
	...
	selectHeader : function(event) {
		// when a header is selected in the list, we will update the name/value
		// text fields as well
		...
		if (selectedTreeItemIndex >= 0) {
			...
			if (treeChildren != null) {
				...
				if (selectedTreeItem != null) {
					var nameCell = selectedTreeItem.getElementsByTagName(
							'treecell').item(0);
					var valueCell = selectedTreeItem.getElementsByTagName(
							'treecell').item(1);
	...
	addParameter : function(name, value) {
		...
		try {
			// You can have multiple parameters - so we won't update existing
